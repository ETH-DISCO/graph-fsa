...
from datasets.gameoflife import GameOfLifeGraph,HexagonalGameOfLifeGraph
...
def train(loader, model, optimizer, device):
    ...
    for data in loader:
        ...
        if model_architecture == "graphfsa":
            ...
            loss = ce_loss(torch.squeeze(output, dim=-1), true_y.to(torch.float))
...
def train_model(params):
    ...
    print(f"----- saved at: {model_save_name} -----")
    
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    timestamp = str(time.time_ns())


    dataset_class = GameOfLifeGraph
    if HEX:
        dataset_class = HexagonalGameOfLifeGraph


    train_dataset = GameOfLifeGraph(GRID_SIZE, params['dataset_size'], steps=STEPS, seed=42, toroidal=TOROIDAL).data
    val_dataset = GameOfLifeGraph(GRID_SIZE, 20, steps=STEPS, seed=43, toroidal=TOROIDAL).data
    test_dataset = GameOfLifeGraph(10, 5, steps=STEPS, seed=44, toroidal=TOROIDAL).data
    ...
    if model_architecture == "graphfsa":
        model = GraphFSA(5, STATES_N)
    
    elif model_architecture == "gnca":
        model = GNNCASimple(hidden=256, activation=torch.sigmoid)
    
    elif model_architecture == "graphchef":
        model = RecurrentGraphChef(1, 2, bounding_parameter=5,state_size=STATES_N, gumbel=True, use_edge_mlp=False, network="mlp")
    ...
    for epoch in pbar:
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model, f"runs/gol/{model_save_name}_{timestamp}.pt")
    ...
    model = torch.load(f"runs/gol/{model_save_name}_{timestamp}.pt")
    ...
    for num_iters in ([1,2,3,4] + list(range(5,101, 5))):
        test_dataset = GameOfLifeGraph(grid_size=10, num_graphs=100, steps=num_iters, seed=100, toroidal=TOROIDAL).data
    ...
    seed = params['seed']
    df.to_csv(f'results/{model_architecture}_{seed}.csv')


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train and evaluate GOL Problem")
    ...
    parser.add_argument("--toroidal", default=True, type=bool)
    ...
    parser.add_argument("--hex", default=False, type=bool)
