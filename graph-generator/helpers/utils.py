...
def compute_class_weights2(loader, STATES_N):
    total_counts = torch.zeros(STATES_N)
    for data in loader:
        c = torch.sum(data.y, dim=0)

        #counts = torch.bincount(data.y.to(torch.long), minlength=STATES_N)
        #total_counts += counts
        total_counts += c[:STATES_N]
